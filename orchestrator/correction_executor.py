...
def execute_correction(
    ...
    *,
    ...
    """Apply one human-approved correction, or surface failure."""
    ...
    if not ok:
        _audit({
            ...
            "ticket_id": ticket_id,
            "case_id": case_id,
            "error": f"token validation failed: {reason}",
        })
        if ticket_id:
            update_ticket(ticket_id, status="correction_failed", failure_reason=reason)
        return {"status": "failed", "error": f"token validation failed: {reason}"}

    field = normalize_field(field)
    before = effective_modern_record(case_id).get(field)
    result = apply_correction(case_id, field, new_value, approval_token)
    if result.get("status") != "applied":
        error = result.get("error", "unknown apply failure")
        _audit({
            "type": "correction_failed",
            "at": at,
            "ticket_id": ticket_id,
